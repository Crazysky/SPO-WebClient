...
/**
 * Fetched property value with metadata
 */
export interface PropertyValue {
  ...
  rawValue: string;
  /** Parsed/formatted value for display */
  displayValue: string;
  /** Original property definition */
  definition: PropertyDefinition;
  /** For indexed properties, the index */
  index?: number;
}

/**
 * Connection data for supply/demand links
 */
export interface ConnectionData {
  /** Connected facility name */
  facilityName: string;
  /** Company name of connected building */
  companyName: string;
  /** Creator/owner */
  createdBy: string;
  /** Price per unit */
  ...
  connected: boolean;
  /** X coordinate of connected building */
  x: number;
  /** Y coordinate of connected building */
  y: number;
}
...
export interface SupplyData {
  /** Resource path */
  path: string;
  /** Resource name (e.g., "Pharmaceutics") */
  name: string;
  /** Meta fluid type */
  metaFluid: string;
  /** Current value (e.g., "2 kg/day") */
  fluidValue: string;
...
/**
 * Service data for retail buildings
 */
export interface ServiceData {
  /** Service name (e.g., "Pharmaceutics") */
  name: string;
...
/**
 * Worker class data (3 tiers: Low=0, Middle=1, High=2)
 */
export interface WorkerData {
  /** Worker class (0=Low, 1=Middle, 2=High) */
  classIndex: number;
  /** Class display name */
  className: string;
  /** Current salary percentage (100 = market rate) */
  salary: number;
  /** Actual salary value */
  salaryValue: number;
...
export interface BuildingDetailsData {
  ...
  /** Security/owner ID */
  securityId: string;
  /** All fetched property values by group */
  groups: Map<string, PropertyValue[]>;
  /** Parsed supply data (if applicable) */
  supplies?: SupplyData[];
  /** Parsed service data (if applicable) */
