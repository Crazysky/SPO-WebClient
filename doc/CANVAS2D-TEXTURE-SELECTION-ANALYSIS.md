...
**Object Atlases** (generated by `src/server/atlas-generator.ts`):

Server-side atlas packing for roads, concrete, and car textures. Unlike the terrain atlas
(fixed 64×32 cells), object atlases use **variable cell dimensions** determined by pre-scanning
all BMPs in the source directory:

| Atlas | Cell Size | Textures | Source Directory |
|-------|-----------|----------|-----------------|
| Road | 64×49 | ~55 | `cache/RoadBlockImages/` |
| Concrete | 68×80 | ~51 | `cache/ConcreteImages/` |
| Car | 64×32 | 88 | `cache/CarImages/` |

Textures are **bottom-aligned** within cells (tall textures anchor at cell bottom, short
textures have empty space above). The manifest stores actual `{x, y, width, height}` per
entry so the client reads correct source rectangles regardless of individual texture size.

API endpoints: `GET /api/object-atlas/:category` (PNG) and `/api/object-atlas/:category/manifest` (JSON).
...
- `src/server/atlas-generator.ts` - Atlas packing (terrain + object atlases)
...
**Date**: 2026-02-09
**Updated**: Added object atlas system (variable cell heights for road/concrete/car atlases)
